...
def overlay_text(frame, text, position=(10, 30), color=(0, 255, 0)):
    ...
    # combined_depthmap = depthmap # np.clip(combined_depthmap, 0, 20)
    # o3d.depth2points(combined_depthmap)
...
def process_photometricstereo(frame):
    ...
    ''' Get depthmap using light vectors '''
    pixel_intensities = frame.reshape(-1, 3) / 255.0
    depthmap = nn.get_depthmap(frame)

    o3d.depth2points(depthmap)

    ''' Captures and Updates Visualizer '''
    img = o3d.capture_image()
    if o3d.visualizer is None or np.all(img == 255):
        o3d.init_visualizer("Open3D")

    colormap = pixel_intensities
    o3d.update_visualizer(colormap)

    end_time = time.time()  # End time
    computation_time = end_time - start_time

    # Overlay computational time on the rendering
    if len(img.shape) == 2 or img.shape[2] != 3:  # Ensure it's in BGR format
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    overlay_text(img, f"Processing Time: {computation_time:.4f} sec", (10, 30))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Process and display in the 3D Rendering frame
        img = process_photometricstereo(frame)
        cv2.imshow('3D Rendering', img)
